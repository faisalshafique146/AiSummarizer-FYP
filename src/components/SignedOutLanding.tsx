    ...
    <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <section aria-labelledby="workflow-heading" className="py-14 sm:py-18">
        <div className="grid gap-8 lg:grid-cols-[260px_1fr] lg:gap-16">
          <div>
            <p className="font-mono text-xs tracking-[0.16em] text-slate-500 uppercase">
              The workflow
            </p>
            <h2
              className="mt-4 text-2xl font-semibold tracking-[-0.025em] text-slate-950"
              id="workflow-heading"
            >
          ...
          <ol className="grid border-t border-slate-300 sm:grid-cols-2 lg:grid-cols-4">
            {workflowSteps.map((step, index) => (
              <li
                className={`border-b border-slate-300 py-6 sm:px-5 lg:border-b-0 ${
                  index % 2 === 1 ? "sm:border-l" : ""
                } ${index > 0 ? "lg:border-l" : ""}`}
                key={step.number}
              >
                <span className="font-mono text-xs text-blue-700">{step.number}</span>
